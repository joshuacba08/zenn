...
flowchart LR
    subgraph Buena["Buena Arquitectura"]
        ...
        B2["Fácil onboarding"]
        B3["Código mantenible"]
        B4["Alta productividad"]
    end
    
    subgraph Mala["Anti-patrón"]
        M1["Mal flujo de trabajo"]
        M2["Difícil onboarding"]
        M3["Código frágil"]
...
<<<<<<< HEAD
### No separar la lógica de negocio de la interfaz de usuario

Uno de los anti-patrones de arquitectura más comunes en el Frontend es no separar la lógica de negocio de la interfaz de usuario. Este anti-patrón se conoce como `Spaghetti Code` y se caracteriza por tener una base de código desorganizada, difícil de mantener y de entender. En este anti-patrón, la lógica de negocio y la interfaz de usuario están mezcladas, lo que hace que sea difícil de testear y de escalar.

#### ¿Qué conocemos como lógica de negocio?

La lógica de negocio es el conjunto de reglas y procesos que definen cómo se realizarán las operaciones de un sistema. En el Frontend, la lógica de negocio se encarga de procesar los datos y de realizar las operaciones necesarias para que la interfaz de usuario funcione correctamente. Por ejemplo, si estamos desarrollando una aplicación de carrito de compras, la lógica de negocio se encargará de sumar los precios de los productos, de calcular el total de la compra y de aplicar los descuentos correspondientes.

#### ¿Qué conocemos como interfaz de usuario?

La interfaz de usuario es la parte visual de una aplicación que interactúa con el usuario. En el Frontend, la interfaz de usuario se encarga de mostrar los datos y de permitir que el usuario interactúe con la aplicación. Por ejemplo, si estamos desarrollando una aplicación de carrito de compras, la interfaz de usuario se encargará de mostrar los productos, de permitir que el usuario seleccione los productos que desea comprar y de mostrar el total de la compra.

#### Solución: separar la lógica de negocio de la interfaz de usuario

Para evitar caer en el anti-patrón de no separar la lógica de negocio de la interfaz de usuario, es importante seguir el principio de `Separation of Concerns` (Separación de Responsabilidades). Este principio consiste en separar las diferentes responsabilidades de un sistema en módulos independientes, de manera que cada módulo se encargue de una sola responsabilidad y que cada responsabilidad esté separada de las demás.

En React JS podemos aplicar este principio utilizando `Hooks` y `Context API` para separar la lógica de negocio de la interfaz de usuario. Por ejemplo, podemos crear un `Hook` que se encargue de manejar la lógica de negocio de una aplicación de carrito de compras y un `Context` que se encargue de manejar la interfaz de usuario de la aplicación. De esta manera, podremos separar las diferentes responsabilidades de la aplicación y evitar caer en el anti-patrón de no separar la lógica de negocio de la interfaz de usuario. También podemos aplicar el concepto de componentes `Presentational` y `Smart` para separar la lógica de presentación de la lógica de negocio.

### No utilizar un patrón de arquitectura (condición necesaria pero no suficiente)

En mi experiencia he tenido la suerte de conocer a muchos desarrolladores habilidosos que sin tener en cuenta los patrones de arquitectura han aplicado de manera intuitiva una buena arquitectura en sus proyectos. Sin embargo, **no utilizar un patrón de arquitectura puede ser un indicio de que estamos aplicando un anti-patrón de arquitectura en el Frontend**. Es decir, no utilizar un patrón de arquitectura no es suficiente para diagnosticar que estamos aplicando un anti-patrón de arquitectura, pero es una condición necesaria que predispone a que aparezcan los anti-patrones.

#### Solución: utilizar un patrón de arquitectura para una primera aproximación

No todo está escrito en piedra, puedes aplicar un patrón de arquitectura en un determinado momento de tu aplicación y luego ir adaptándolo a tus necesidades. Algunos de los patrones de arquitectura más utilizados en el Frontend son `MVC` (Model-View-Controller), `MVVM` (Model-View-ViewModel) y `Flux`. Cada uno de estos patrones tiene sus propias ventajas y desventajas, por lo que es importante evaluar cuál es el más adecuado para tu proyecto.

### No aplicar buenas prácticas de programación

Este quizá es un punto muy controversial ya que las `buenas prácticas de programación` pueden variar de un desarrollador a otro y en muchos casos ser subjetivas. Sin embargo, el simple hecho de que el desarrollador o el equipo se plantee aplicar buenas prácticas de programación ya es un indicio de que están preocupados por la calidad del software y la productividad del equipo.

#### Solución: definir una guía de estilo de código validada por todo el equipo

Personalmente, más allá de lo que está escrito en libros y artículos, considero que las buenas prácticas son un concenso entre los miembros del equipo. Por eso, es importante sentarse a discutir y definir una guía de estilo con la que todo el equipo se sienta cómodo.

Evita las discusiones interminables sobre si se deben utilizar `tabs` o `espacios`, `punto y coma` o `no punto y coma`, `camelCase` o `snake_case`. Define una guía de estilo que sea clara, concisa y que se adapte a las necesidades del equipo. Puedes utilizar herramientas como `ESLint` y `Prettier` para automatizar la aplicación de la guía de estilo y evitar discusiones innecesarias. Además, puedes utilizar herramientas como `Husky` y `Lint-Staged` para prevenir que se suban cambios que no cumplan con la guía de estilo.

## Conclusión

Antes que nada, si has llegado hasta acá te quiero agradecer por haber leído este artículo. Espero que te haya sido de utilidad o de tu agrado. No dudes en dejarme un comentario si tienes alguna duda o sugerencia.

Quiero terminar con un gráfico que guarda relación 
...
flowchart TB
    subgraph StateStrategy["Estrategia de Estado"]
        subgraph Local["Estado Local"]
            ...
            L2["useReducer - Lógica compleja local"]
        end
...
flowchart TD
    ...
    subgraph Proceso["Proceso de decisión"]
        D{"¿Cuántas veces\nhas repetido\neste código?"}
        D -->|1 vez| E["Implementa directamente"]
        D -->|2 veces| F["Considera si vale la pena abstraer"]
        D -->|3+ veces| G["Es momento de crear una abstracción"]
    end
```
...
```mermaid
flowchart LR
    subgraph Evolución["Evolución del código"]
        A["Funciona"] --> B["Es correcto"]
        B --> C["Es rápido"]
    end
    
    C --> D{"¿Hay patrón\nrepetido 3+ veces?"}
    D -->|Sí| E["Identifica la abstracción correcta"]
    D -->|No| F["Mantén simple, no anticipes"]
...
flowchart LR
    subgraph Journey["El Camino"]
        ...
        D --> E["Establecer guías"]
        E --> F["Revisar periódicamente"]
        F --> A
    end
```
...
>>>>>>> 91c5ae28e40c9d05bde429ca76b6389d07ec5eda
